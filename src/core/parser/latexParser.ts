...
export class LatexParser {
  ...
  private customCommands: Map<
    string,
    { name: string; numArgs: number; optionalArg?: string; definition: string }
  >;
   ...
   */
  private validateDelimiterPairs(
    text: string,
    pattern: RegExp,
    offset: number
  ): boolean {
  ...
  private handleNewTheorem(text: string): string {
    const newTheoremPattern =
      /\\newtheorem\{[^}]+\}(?:\[[^]]+\])?\{[^}]+\}(?:\[[^]]+\])?/g;
    return text.replace(newTheoremPattern, (match): string =>
      this.theoremParser.handleNewTheorem(match)
    );
  }
  ...
  private processTextInMath(text: string): string {
    return text.replace(/\\text\{([^}]*)\}/g, (match, content): string => {
      if (!content) {
  ...
  private processEquationTags(text: string): string {
    return text.replace(/\\tag\{([^}]+)\}/g, (match, tag): string => {
      if (!tag) {
  ...
  private processMathOperators(text: string): string {
    for (const [name, config] of Object.entries(MATH_OPERATORS)) {
      ...
      text = text.replace(pattern, (match, sub, sup): string => {
        // If no valid operator config, return the original matched text
        if (!config) {
          return match;
        }

        const operator = (config as MathOperator).hasLimits
          ? `\\operatorname*{${name}}`
          : `\\operatorname{${name}}`;
        if (!sub && !sup) {
          return operator;
   ...
   */
  private processMathEnvironment(
    env: string,
    content: string,
  ...
  private processLabelsAndRefs(text: string, options: ParserOptions): string {
    try {
      ...
      const labels = new Map<
        string,
        { number: number; type?: string; name?: string }
      >();
      const counters = {
        equation: 1,
        section: 1,
      ...
      if (options.handleRefs === 'resolve') {
        ...
        while ((match = labelRegex.exec(text)) !== null) {
          ...
          const type =
            context.match(
              /\\begin\{(equation|figure|table|section)\*?\}/
            )?.[1] || 'equation';
        ...
        const refReplacements: [RegExp, (label: string) => string][] = [
          ...
          [
            ...
            (label): string => {
              ...
              return info
                ? REFERENCE_FORMATS.CUSTOM.ref.replace('$label', label)
                : '(?)';
            },
          ],
          // Equation references
          [
            ...
            (label): string => {
              const info = labels.get(label);
              return info
                ? REFERENCE_FORMATS.CUSTOM.eqref.replace(
                    '$number',
          ...
          [
            ...
            (label): string => {
              const info = labels.get(label);
              return info?.name
                ? REFERENCE_FORMATS.CUSTOM.nameref.replace('$name', info.name)
                : '(?)';
            },
          ],
          // Auto references
          [
            ...
            (label): string => {
              const info = labels.get(label);
              const format =
                REFERENCE_FORMATS.CUSTOM?.autoref ??
                REFERENCE_FORMATS.DEFAULT ??
                '(?)';
              return info
                ? format
        ...
        processedText = processedText.replace(
          /\\(?:eq|page|name|auto|v)?ref\{([^}]+)\}/g,
          (match, label) => {
            if (!label) {
              console.warn(`Empty reference found: ${match}`);
              return match;
            }
            const refType = match.startsWith('\\eqref')
              ? 'EQUATION'
              : match.startsWith('\\pageref')
                ? 'PAGE'
                : match.startsWith('\\nameref')
                  ? 'NAME'
                  : 'DEFAULT';
            return REFERENCE_FORMATS[refType].replace(
              /\$(?:label|number|name)/,
              label
            );
          }
  ...
  private processSpecialCommands(text: string, options: ParserOptions): string {
    ...
    const leftRightPatterns = [
      {
        pattern: /\\left\.\s*([^\\]+?)\\right([^.\s])/g,
        replace: '$1\\right$2',
      },
      {
        pattern: /\\left([^.\s])\s*([^\\]+?)\\right\./g,
        replace: '\\left$1$2',
      },
      ...
      {
        pattern: /\\left\.\s*(\\begin\{[^}]+\}.*?\\end\{[^}]+\})\\right\./g,
        replace: '$1',
      },
    ];
  ...
  private processArray(content: string, colSpec: string): string {
    ...
    const cols = colSpec
      .split('')
      .filter(
        (c): c is keyof typeof ARRAY_COLUMN_TYPES => c in ARRAY_COLUMN_TYPES
      );
   ...
   */
  private formatArrayCell(
    content: string,
    colType: keyof typeof ARRAY_COLUMN_TYPES
  ): string {
  ...
  private processEnvironments(
    text: string,
    options: ParserOptions = { direction: 'latex-to-obsidian' }
  ): string {
    let result = text;
    let currentIndex = 0;
    ...
    while (currentIndex < text.length) {
      try {
        const [processedContent, nextIndex] = this.parseEnvironment(
          text,
          currentIndex
        );
        if (processedContent) {
          const envMatch = processedContent.match(
            /\\begin\{([^}]+)\}([\s\S]*?)\\end\{\1\}/
          );
          if (envMatch) {
            const [fullMatch, env, content] = envMatch;
            const processedMathContent = this.processMathEnvironment(
              env,
              content,
              options
            );
   ...
   */
  private convertEnvironmentToDisplay(
    env: string,
    content: string,
  ...
  public parseObsidianToLatex(
    text: string,
    options: ParserOptions = { direction: 'obsidian-to-latex' }
  ): string {
    try {
      // Reset counters and maps
      this.equationCounter = 0;
      ...
      if (finalOptions.convertEnvironments) {
        ...
        processedText = this.replaceNestedEnvironments(
          processedText,
          envPattern,
          finalOptions
        );
      ...
      processedText = Array.isArray(mathDelimiterResult)
        ? mathDelimiterResult[0]
        : mathDelimiterResult;
  ...
  public parseWithRecovery(content: string): string {
    try {
      ...
      if (error instanceof DelimiterMismatchError) {
        ...
        console.warn(
          `Attempting to recover from delimiter mismatch: ${error.message}`
        );
        return ErrorRecovery.handleDelimiterMismatch(
          content,
          this.position,
          error.message
        );
        ...
        console.warn(
          `Attempting to recover from environment mismatch: ${error.message}`
        );
        return ErrorRecovery.handleEnvironmentMismatch(
          content,
          this.position,
          error.message
        );
   ...
   */
  private replaceNestedEnvironments(
    text: string,
    envPattern: string,
    options: ParserOptions
  ): string {
      ...
      while ((match = envRegex.exec(result)) !== null) {
        ...
        if (command === 'begin') {
          ...
          if (envStack.length > 0) {
            ...
            if (
              isValidEnvironment(outerEnv) &&
              isValidEnvironment(env) &&
              !VALID_NESTING[outerEnv]?.includes(env)
            ) {
          ...
          const content = result.slice(
            envStack.length === 0
              ...
              : envStack[envStack.length - 1].length,
            index
          );
          const converted = this.convertEnvironmentToDisplay(
            env,
            content,
            options
          );
  ...
  private parseEnvironment(
    ...
    if (this.environmentStack.length > 0) {
      ...
      if (
        ...
        !VALID_NESTING[outerEnv]?.includes(envName)
      ) {
    ...
    const processedContent = this.processEnvironmentContent(
      envName,
      envContent
    );
  ...
  private isValidEnvironment(envName: string): boolean {
    return (
      envName in ENVIRONMENT_MAPPINGS ||
  ...
  private processEnvironmentContent(envName: string, content: string): string {
    ...
    switch (envName.toLowerCase()) {
      case 'equation':
      case 'equation*':
        return this.processMathEnvironment(envName, content, {
  ...
  private replaceMathDelimiters(text: string): string | [string, boolean] {
    ...
    for (const [pattern] of MATH_DELIMITERS.DISPLAY) {
      processedText = processedText.replace(pattern, (match, content) => {
        ...
        if (
          !this.validateDelimiterPairs(match, pattern as RegExp, this.position)
        ) {
      ...
      Object.entries(COMMON_MACROS).forEach(([macro, definition]) => {
        ...
        processedText = processedText.replace(macroRegex, (match, ...args) => {
          ...
          if (macroArgs.length !== definition.numArgs) {
            console.warn(
              `Macro expansion failed for ${match}: expected ${definition.numArgs} args, got ${macroArgs.length}`
            );
