...
export const ERROR_MESSAGES = {
  UNKNOWN_ENVIRONMENT: (env: string): string => `Unknown environment: ${env}`,
  INVALID_NESTING: (outer: string, inner: string): string =>
    `Invalid nesting: ${inner} inside ${outer}`,
  MISSING_END: (env: string): string => `Missing \\end{${env}}`,
  UNMATCHED_DELIMITER: (delim: string): string =>
    `Unmatched delimiter: ${delim}`,
  INVALID_REFERENCE: (ref: string): string => `Invalid reference: ${ref}`,
  LABEL_ERROR: (label: string): string => `Error processing label: ${label}`,
  INVALID_MACRO: (name: string): string => `Invalid macro: ${name}`,
  MACRO_ARG_MISMATCH: (name: string, expected: number, got: number): string =>
    `Macro ${name} expects ${expected} arguments, got ${got}`,
  CITATION_ERROR: (key: string): string => `Error processing citation: ${key}`,
  NESTED_ERROR: (outer: string, inner: string): string =>
    `Invalid nesting: ${inner} cannot be nested inside ${outer}`,
  UNMATCHED_ENVIRONMENT: (env: string): string =>
    `Unmatched environment: ${env}`,
};
...
export type BuiltInMacroDefinition = BaseMacroDefinition;
