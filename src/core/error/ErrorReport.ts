...
export class ErrorReportGenerator {
  public static generateMarkdownReport(report: ErrorReport): string {
    ...
    const recommendations =
      ErrorReportGenerator.generateRecommendations(report);
  ...
  private static generateRecommendations(report: ErrorReport): string[] {
    ...
    const [mostCommonCategory, mostCommonCount] = errorCategories.sort(
      ([, a], [, b]) => b - a
    )[0];
    ...
    const hasMultipleCategories =
      errorCategories.filter(([, count]) => count > 0).length > 1;
