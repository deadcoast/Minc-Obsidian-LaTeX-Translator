...
export class DelimiterMismatchError extends LaTeXParserError {
  constructor(expected: string, found: string | null, position: number) {
    super(
      `Expected delimiter "${expected}" but found "${found || 'EOF'}" at position ${position}`
    );
...
export class EnvironmentMismatchError extends LaTeXParserError {
  constructor(expected: string, found: string | null, position: number) {
    super(
      `Expected environment "${expected}" but found "${found || 'EOF'}" at position ${position}`
    );
...
export class InvalidNestingError extends LaTeXParserError {
  constructor(outer: string, inner: string, position: number) {
    super(
      `Invalid nesting: "${inner}" cannot be nested inside "${outer}" at position ${position}`
    );
...
export class RecoveryAttemptError extends LaTeXParserError {
  constructor(message: string, originalError: Error) {
    super(
      `Recovery attempt failed: ${message}. Original error: ${originalError.message}`
    );
...
export class ErrorRecovery {
   ...
   */
  static handleDelimiterMismatch(
    text: string,
    position: number,
    expected: string
  ): string {
   ...
   */
  static handleEnvironmentMismatch(
    text: string,
    position: number,
    environment: string
  ): string {
  ...
  static canNest(outer: string, inner: string): boolean {
    ...
    return outer in validNesting
      ? validNesting[outer as keyof ValidNestingRules].includes(inner)
      : false;
  }
