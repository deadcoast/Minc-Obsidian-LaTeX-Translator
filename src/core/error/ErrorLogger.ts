...
export class ErrorLogger {
    ...
    private formatLogEntry(errorDetails: ErrorDetails): string {
        ...
        const context = errorDetails.context ? JSON.stringify(errorDetails.context) : '';
        
        return [
            `[${timestamp}] ${errorDetails.severity.toUpperCase()} - ${errorDetails.category}`,
            `Message: ${errorDetails.message}`,
            context ? `Context: ${context}` : '',
            errorDetails.stackTrace ? `Stack Trace: ${errorDetails.stackTrace}` : '',
            errorDetails.suggestions?.length ? `Suggestions: ${errorDetails.suggestions.join(', ')}` : '',
            '---'
        ].filter(Boolean).join('\n') + '\n';
    }
