...
interface CodeMirrorEditor extends Editor {
  cm: {
    decorateRange: (
      ...
      options: { attributes: { class: string; title: string } }
    ) => RangeSet<Decoration>;
    setGutterMarker: (
      line: number,
      gutter: string,
    ...
    state: {
      field: <T>(field: T) => T;
      doc: {
        line: (line: number) => { from: number };
      };
    };
    dispatch: (transaction: { effects?: Extension[] }) => void;
  };
}
...
export class ErrorHighlighter {
  ...
  private markers: RangeSet<Decoration>[] = [];
  ...
  private shouldShowError(severity: ErrorSeverity | undefined): boolean {
    ...
    const minSeverityEnum =
      ErrorSeverity[minSeverity.toUpperCase() as keyof typeof ErrorSeverity];
    ...
    return (
      severityLevels.indexOf(effectiveSeverity) >=
      severityLevels.indexOf(minSeverityEnum)
    );
  }
  ...
  private addUnderline(error: ErrorRange, className: string): void {
    ...
    const decoration = (this.editor as CodeMirrorEditor).cm.decorateRange(
      range,
      {
        attributes: {
  ...
  private addGutterMarker(error: ErrorRange): void {
    ...
    const gutterMarker = (this.editor as CodeMirrorEditor).cm.setGutterMarker(
      error.from.line,
  ...
  private addSquiggly(error: ErrorRange, className: string): void {
    ...
    const view = (this.editor as CodeMirrorEditor).cm;
    if (!view) {
      return;
    }
      ...
      add: [
        {
          ...
          value: view.Decoration.mark({
            class: `${className} latex-translator-error-squiggly`,
          }),
        },
      ],
    });
  ...
  private addBorder(error: ErrorRange, className: string): void {
    ...
    const decoration = Decoration.mark({
      ...
      view.state.doc.line(error.from.line + 1).from + error.from.ch,
      view.state.doc.line(error.to.line + 1).from + error.to.ch
    );

    view.dispatch({
      effects: StateEffect.appendConfig.of([
        EditorView.decorations.of(RangeSet.of([decoration])),
      ]),
    });
    this.markers.push(decoration);
  }
