...
export class HelpManager {
  ...
  public async renderHelpContent(
    containerEl: HTMLElement,
    section?: string,
    component?: Component
  ): Promise<void> {
    const content = section
      ? HelpContent[section as keyof typeof HelpContent]
      : HelpContent.overview;
    const activeView =
      component ?? this.app.workspace.getActiveViewOfType(MarkdownView);
    if (!activeView) {
...
export class HelpModal extends Modal {
  ...
  onOpen(): void {
    ...
    HelpManager.getInstance(this.app)
      .getHelpSections()
      .forEach((section) => {
  ...
  private async renderContent(containerEl: HTMLElement): Promise<void> {
    containerEl.empty();
    await HelpManager.getInstance(this.app).renderHelpContent(
      containerEl,
      this.currentSection,
  ...
  private filterSections(query: string): void {
    const navItems =
      this.contentEl.querySelectorAll<HTMLElement>('.help-nav-item');
    navItems.forEach((item) => {
      const title =
        item.querySelector('.help-nav-title')?.textContent?.toLowerCase() || '';
