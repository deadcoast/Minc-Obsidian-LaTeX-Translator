...
export class LatexTranslatorSettingsTab extends PluginSettingTab {
  ...
  private availableSettings: LatexTranslatorSettings &
    Required<Pick<LatexTranslatorSettings, 'uiSettings'>>;
  ...
  constructor(app: App, plugin: ILatexTranslatorPlugin) {
    ...
    this.availableSettings = {
      ...DEFAULT_SETTINGS, // First spread the default settings to ensure we have all required properties
  ...
  private addDefaultSettingsReference(containerEl: HTMLElement): void {
    ...
    defaultSettingsContainer.createEl('summary', {
      text: 'Click to view all available settings and their default values',
    });
    const pre = defaultSettingsContainer.createEl('pre', {
      cls: 'settings-reference',
    });
    pre.createEl('code', {
      text: JSON.stringify(this.availableSettings, null, 2),
    });
  }
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value) => {
            this.settings.uiSettings.previewPanelPosition = value as
              | 'right'
              | 'bottom';
            await this.plugin.saveSettings();
          })
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Delay in milliseconds before updating the preview (when auto-update is enabled)'
      )
      .addSlider((slider) =>
        slider
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value) => {
            this.settings.uiSettings.errorHighlightStyle = value as
              | 'underline'
              | 'background'
              | 'gutter';
            await this.plugin.saveSettings();
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value) => {
            this.settings.uiSettings.logDetailLevel = value as
              | 'basic'
              | 'detailed'
              | 'debug';
            await this.plugin.saveSettings();
  ...
  private addEnvironmentSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(
            this.settings.environmentConversion.preserveOriginalOnUnknown
          )
          .onChange(async (value) => {
            this.settings.environmentConversion.preserveOriginalOnUnknown =
              value;
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Add custom environment mappings (one per line, format: source=target)'
      )
      .addTextArea((text) => {
        text
          .setPlaceholder('theorem=callout-theorem\nproof=callout-proof')
          .setValue(
            Object.entries(this.settings.environmentConversion.customMappings)
              .map(([k, v]) => `${k}=${v}`)
              .join('\n')
          )
          .onChange(async (value) => {
            try {
              ...
              value.split('\n').forEach((line) => {
                ...
                if (source && target) {
                  ...
                  if (
                    /^[a-zA-Z][a-zA-Z0-9*]*$/.test(sanitizedSource) &&
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value) => {
            this.settings.labelAndReference.referenceHandling = value as
              | 'ignore'
              | 'placeholder'
              | 'autoNumber';
            await this.plugin.saveSettings();
      ...
      new Setting(containerEl)
        ...
        .addText((text) =>
          text
            .setPlaceholder('1')
            .setValue(
              this.settings.labelAndReference.autoNumbering.startEquation.toString()
            )
            .onChange(async (value) => {
              const num = parseInt(value);
              if (!isNaN(num)) {
                this.settings.labelAndReference.autoNumbering.startEquation =
                  num;
                await this.plugin.saveSettings();
              }
            })
        ...
        .addText((text) =>
          text
            .setPlaceholder('1')
            .setValue(
              this.settings.labelAndReference.autoNumbering.startFigure.toString()
            )
            .onChange(async (value) => {
              const num = parseInt(value);
              if (!isNaN(num)) {
  ...
  private addCitationSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Format for basic \\cite{} commands. Available placeholders: $key, $author, $year, $title'
      )
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            this.settings.citation.defaultFormat = validateAndSanitizeFormat(
              value,
              allowedPlaceholders
            );
            await this.plugin.saveSettings();
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Add custom citation formats (one per line, format: command=template). Available placeholders: $key, $author, $year, $title'
      )
      .addTextArea((text) =>
        text
          .setPlaceholder('citet=$author ($year)\ncitep=[$key]')
          .setValue(
            Object.entries(this.settings.citation.customFormats)
              .map(([k, v]) => `${k}=${v}`)
              .join('\n')
          )
          .onChange(async (value) => {
            try {
              ...
              value.split('\n').forEach((line) => {
                ...
                if (cmd && template) {
                  ...
                  const sanitizedTemplate = validateAndSanitizeFormat(
                    template.trim(),
                    allowedPlaceholders
                  );
  ...
  private addBatchOperationSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl).setName('Batch Operations').setHeading();
    ...
    // Hotkeys section
    new Setting(containerEl).setName('Batch Operation Hotkeys').setHeading();
